...
export default function Sidebard({
  ...
  const { escenariosFiltrados, busqueda, setBusqueda } = useFiltro();
      ...
      <div className="p-3 md:p-4 border-b border-gray-200">
        <div className="flex flex-col md:flex-row md:items-center gap-2 md:gap-4">
          ...
          <div className="relative flex-1">
            ...
            />
            <svg
              className="absolute left-3 top-2.5 md:top-3 w-4 h-4 md:w-5 md:h-5 text-gray-400"
              fill="none"
              stroke="currentColor"
              viewBox="0 0 24 24"
            >
          ...
          <button
            onClick={isOpen}
            className="flex items-center rounded-md justify-center w-10 h-10 bg-gray-300 cursor-pointer hover:bg-gray-400 transition-colors"
          >
            <Funnel strokeWidth={2.5} color="green" />
          </button>
      ...
      <div className="flex-1 overflow-y-auto p-3 md:p-4">
        {loading ? (
          ...
          <div className="text-center text-gray-500 mt-10 md:mt-20">
            <svg
              className="w-16 h-16 md:w-24 md:h-24 mx-auto text-gray-300 mb-4"
              fill="none"
              stroke="currentColor"
              viewBox="0 0 24 24"
            >
          ...
          <div
            className={`grid gap-3 md:gap-4 transition-all duration-500 ${
              showMap
                ? "grid-cols-1 sm:grid-cols-2"
                : "grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4"
            }`}
              ...
              >
                ...
                <div className="relative h-40 md:h-48 overflow-hidden">
                  ...
                  <div className="absolute top-2 md:top-3 left-2 md:left-3">
                    <span
                      className={`px-2 md:px-3 py-1 text-xs font-semibold rounded-full shadow-lg ${
      ...
      <div className="p-2 md:p-3 border-t border-gray-200 bg-gray-50">
        <div className="flex items-center justify-between text-xs md:text-sm">
          <span className="text-gray-600">
            <span className="font-semibold">{escenariosFiltrados.length}</span>{" "}
            de <span className="font-semibold">{escenarios.length}</span>
          </span>
